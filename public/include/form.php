...
function form_respond( $alert, $message ) {
	if( !headers_sent() ) {
		header('Content-Type: application/json; charset=utf-8');
	}
	echo json_encode( array( 'alert' => $alert, 'message' => $message ) );
	exit;
}
...
/*-------------------------------------------------
	Captcha Enforcement (Server-Side)
---------------------------------------------------*/

// Set to true to require either reCaptcha or hCaptcha on every form submission.
// Server-side enforcement; cannot be bypassed by removing form fields client-side.
// Replaces the old client-side `force_recaptcha` hidden input mechanism.
...
if( $_SERVER['REQUEST_METHOD'] == 'POST' ) {
	...
	$botpassed = isset( $submits[ $prefix . 'botcheck' ] ) && empty( $submits[ $prefix . 'botcheck' ] );

	if( !$botpassed ) {
		form_respond( 'error', $message['error_bot'] );
	}
	...
	foreach( $submits as $spam_submit ) {
		if( spam_keyword_check( $spam_submit, $spam_keywords ) || spam_url_check( $spam_submit ) > $allowed_urls ) {
			...
			form_respond( 'success', $message['success'] );
		}
	}


	/*-------------------------------------------------
		Server-Side Captcha Enforcement
	---------------------------------------------------*/

	if( $require_captcha && !isset( $submits['g-recaptcha-response'] ) && !isset( $submits['h-captcha-response'] ) ) {
		form_respond( 'error', $message['captcha_error'] );
	}
	...
	if( isset( $submits['g-recaptcha-response'] ) && !isset( $submits['h-captcha-response'] ) ) {
		...
		if ( !isset( $g_response->success ) || $g_response->success !== true ) {
			form_respond( 'error', $message['captcha_invalid'] );
		}
	}
	...
	if( isset( $submits['h-captcha-response'] ) ) {
		...
		if ( !isset( $h_response->success ) || $h_response->success !== true ) {
			form_respond( 'error', $message['captcha_invalid'] );
		}
	}
	...
	if( !empty( $ar_matches[1] ) ) {
		foreach( $ar_matches[1] as $ar_key => $ar_value ) {
			if( !isset( $submits[ $ar_value ] ) ) {
				$ar_message = str_replace( '{' . $ar_value . '}', '', $ar_message );
				continue;
			}
	...
	$referrer_raw = isset( $_SERVER['HTTP_REFERER'] ) ? $_SERVER['HTTP_REFERER'] : '';
	$referrer_safe = $referrer_raw ? htmlspecialchars( $referrer_raw, ENT_QUOTES, 'UTF-8' ) : '';
	$referrer = $referrer_safe ? '<br><br><br>This Form was submitted from: ' . $referrer_safe : '';
	...
	if( $sendEmail == true ):
		...
		error_log( 'Canvas form.php PHPMailer send failed: ' . $mail->ErrorInfo );
		form_respond( 'error', $message['error'] );
	endif;

} else {
	form_respond( 'error', $message['error_unexpected'] );
}
